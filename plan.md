...
- [ ] 下一步：依 `docs/OWNER_ACCEPTANCE_2026-06-10.md` 跑 local smoke 與 owner manual acceptance，測完後只針對 observed concern/fail 定義修正 DONE
...
- [x] 支援 Resting HR 產生初步 Zone 2 參考範圍、來源 / 初步估算標示、套用參考範圍、重設回預設界線
...
- [x] Training Classification v3.1 feedback store dependency wiring
- [x] Training Classification v3.1 live recorder / classification snapshot flow
- [x] Training Classification v3.1 feedback saved state / duplicate handling
- [ ] Training Classification v3.1 下一步：owner acceptance / manual test checklist 前先定義窄切片
...
| 2026-06-10 | 新增 Core-only spec resolution metadata 型別：`TrainingSpecResolution`、`EvidenceLayer`、`SourceRoleLayer`、`SourceRoleReason`，並補 Codable/envelope tests；尚未接入現有 analyzer 或 UI |
| 2026-06-10 | 將 `TrainingSpecResolution` 接入 `TrainingMetricMetadata`：新欄位預設 Evidence Map + no source-role，舊 JSON 不帶欄位仍可 decode，新 metadata 可帶 Apple Health source-role |
| 2026-06-10 | 將 Apple Health-backed metric metadata 標上 source-role：VO2 max、HRR、跑步/自行車功率、路線脈絡會攜帶 Apple Health role matrix 與對應 reason |
